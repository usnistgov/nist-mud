...
curl -u admin:admin -v -X PUT  -k  --header "Content-Type:application/json" --data @cpe-vlans.json http://$CONTROLLER_ADDR:8181/restconf/config/nist-network-topology:topology
