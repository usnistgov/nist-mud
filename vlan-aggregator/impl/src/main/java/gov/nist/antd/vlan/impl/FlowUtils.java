...
class FlowUtils {
    ...
    private static MatchBuilder createVlanPresentMatch(MatchBuilder matchBuilder) {
        VlanMatchBuilder vlanMatchBuilder = new VlanMatchBuilder();
        VlanIdBuilder vlanIdBuilder = new VlanIdBuilder();
    ...
    public static FlowBuilder createVlanArpMatchSendToControllerAndGoToTable(Short tableId,
            int timeout, FlowId flowId, FlowCookie flowCookie) {

        FlowBuilder flowBuilder = new FlowBuilder().setTableId(tableId).setFlowName("arpMatchSendToControllerAndGoToTable")
                .setId(flowId).setKey(new FlowKey(flowId)).setCookie(flowCookie);
    ...
    public static FlowBuilder createVlanAndPortMatchSendToPort(String inputPort,
            String outputPort, int vlanTag, Short tableId,
            Integer cacheTimeout, FlowId flowId, FlowCookie flowCookie) {

        FlowBuilder flowBuilder = new FlowBuilder().setTableId(tableId).setFlowName
                ("vlanAndPortMatchSendToPort")
                .setId(flowId).setKey(new FlowKey(flowId)).setCookie(flowCookie);
        MatchBuilder matchBuilder = new MatchBuilder();
        FlowUtils.createInPortMatch(matchBuilder, new NodeConnectorId(inputPort));
        FlowUtils.createVlanMatch(matchBuilder, vlanTag);
        Instruction ins = FlowUtils.createSendToPortInstruction( outputPort);
        ArrayList<Instruction> instructions = new ArrayList<>();
        instructions.add(ins);
        InstructionsBuilder isb = new InstructionsBuilder();
        isb.setInstruction(instructions);
