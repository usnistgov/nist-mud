...
public abstract class PacketUtils {
     ...
     */
    private static final int DST_MAC_END_POSITION = DST_MAC_START_POSITION
            + MAC_ADDRESS_SIZE;
     ...
     */
    private static final int SRC_MAC_END_POSITION = SRC_MAC_START_POSITION
            + MAC_ADDRESS_SIZE;
    ...
    private static final int PACKET_OFFSET_TCP_DST_PORT = PACKET_OFFSET_TCP_SRC_PORT
            + 2;

    private static final Logger LOG = LoggerFactory
            .getLogger(PacketUtils.class);
    ...
    public static byte[] extractDstMac(final byte[] payload) {
        return Arrays.copyOfRange(payload, DST_MAC_START_POSITION,
                DST_MAC_END_POSITION);
    }
    ...
    public static byte[] extractSrcMac(final byte[] payload) {
        return Arrays.copyOfRange(payload, SRC_MAC_START_POSITION,
                SRC_MAC_END_POSITION);
    }
    ...
    public static byte[] extractEtherType(final byte[] payload) {
        return Arrays.copyOfRange(payload, ETHER_TYPE_START_POSITION,
                ETHER_TYPE_END_POSITION);
    }
    ...
    public static String extractSrcIpStr(final byte[] rawPacket) {
        final byte[] ipSrcBytes = Arrays.copyOfRange(rawPacket,
                PACKET_OFFSET_IP_SRC, PACKET_OFFSET_IP_SRC + 4);
        String pktSrcIpStr = null;
        try {
            pktSrcIpStr = InetAddress.getByAddress(ipSrcBytes).getHostAddress();
        } catch (UnknownHostException e) {
            LOG.error("Exception getting Src IP address [{}]", e.getMessage(),
                    e);
        }
        return pktSrcIpStr;
    }
    ...
    public static String extractDstIpStr(final byte[] rawPacket) {
        final byte[] ipDstBytes = Arrays.copyOfRange(rawPacket,
                PACKET_OFFSET_IP_DST, PACKET_OFFSET_IP_DST + 4);
        String pktDstIpStr = null;
        try {
            pktDstIpStr = InetAddress.getByAddress(ipDstBytes).getHostAddress();
        } catch (UnknownHostException e) {
            LOG.error("Exception getting Dst IP address [{}]", e.getMessage(),
                    e);
        }
        return pktDstIpStr;
    }
