 ...
 * DHCP protocol. Subclasses create the specific packets used at each stage of
 * the negotiation.
 */
public abstract class DhcpPacket {

    protected static Logger LOG = LoggerFactory.getLogger(DhcpPacket.class);
    ...
    /**
     * The code type used to identify an Ethernet MAC address in the Client-ID
     * field.
     */
     ...
     */
    protected static final short DHCP_SERVER_IDENTIFIER = 54;
    protected InetAddress mServerIdentifier;
    /**
     * DHCP Optional Type: DHCP Parameter List
     */
    protected static final short DHCP_PARAMETER_LIST = 55;
    protected byte[] mRequestedParams;
    /**
     * DHCP Optional Type: DHCP MESSAGE
     */
    protected static final short DHCP_MESSAGE = 56;
    protected String mMessage;
    /**
     * DHCP Optional Type: DHCP Renewal Time Value
     */
    protected static final short DHCP_RENEWAL_TIME = 58;
    /**
     * DHCP Optional Type: Vendor Class Identifier
     */
    protected static final short DHCP_VENDOR_CLASS_ID = 60;
    /**
     * DHCP Optional Type: DHCP Client Identifier
     */
    protected static final short DHCP_CLIENT_IDENTIFIER = 61;

    protected static final short DHCP_MUD_URL = 161;

    protected static Inet4Address INET4_ADDRESS_ANY;

    protected static Inet4Address INET4_ADDRESS_ALL;
    ...
    /**
     * The IP address of the client host. This address is typically proposed by
     * the client (from an earlier DHCP negotiation) or supplied by the server.
     */
    ...
    static {
        try {
            INET4_ADDRESS_ANY = (Inet4Address) InetAddress.getByName("0.0.0.0");
            INET4_ADDRESS_ALL = (Inet4Address) InetAddress
                    .getByName("255.255.255.255");
        } catch (Exception ex) {
            LOG.error("Exception resolving inet address 0.0.0.0", ex);
            INET4_ADDRESS_ANY = null;
            INET4_ADDRESS_ALL = null;
        }
    }

    /**
     * Asks the packet object to create a ByteBuffer serialization of the packet
     * for transmission.
     */
    ...
    protected DhcpPacket(int transId, InetAddress clientIp, InetAddress yourIp,
            InetAddress nextIp, InetAddress relayIp, byte[] clientMac,
            boolean broadcast) {
        mTransId = transId;
        mClientIp = clientIp;
        mYourIp = yourIp;
    ...
    /**
     * Creates a new L3 packet (including IP header) containing the DHCP udp
     * packet. This method relies upon the delegated method finishPacket() to
     * insert the per-packet contents.
     */
    protected void fillInPacket(int encap, InetAddress destIp,
        ...
        buf.put(mClientMac);
        buf.position(buf.position() + (16 - mClientMac.length) // pad addr to 16
                                                               // bytes
    ...
    /**
     * Converts a signed short value to an unsigned int value. Needed because
    ...
    /**
     * Performs an IP checksum (used in IP header and across UDP payload) on the
     * specified portion of a ByteBuffer. The seed allows the checksum to
     * commence with a specified value.
     */
     ...
     */
    protected void addTlv(ByteBuffer buf, short type, byte value) {
        buf.put((byte) type);
        buf.put((byte) 1);
        buf.put(value);
    }
     ...
     */
    protected void addTlv(ByteBuffer buf, short type, byte[] payload) {
        if (payload != null) {
            buf.put((byte) type);
     ...
     */
    protected void addTlv(ByteBuffer buf, short type, InetAddress addr) {
        if (addr != null) {
     ...
     */
    protected void addTlv(ByteBuffer buf, short type, List<InetAddress> addrs) {
     ...
     */
    protected void addTlv(ByteBuffer buf, short type, Integer value) {
        if (value != null) {
            buf.put((byte) type);
            buf.put((byte) 4);
     ...
     */
    protected void addTlv(ByteBuffer buf, short type, String str) {
        if (str != null) {
            buf.put((byte) type);
    ...
    /**
     * Reads a four-octet value from a ByteBuffer and construct an IPv4 address
     * from that value.
     */
    ...
    /**
     * Creates a concrete DhcpPacket from the supplied ByteBuffer. The buffer
     * may have an L2 encapsulation (which is the full EthernetII format
     * starting with the source-address MAC) or an L3 encapsulation (which
     * starts with the IP header). <br>
     * A subset of the optional parameters are parsed and are stored in object
     * fields.
     */
    public static DhcpPacket decodeFullPacket(ByteBuffer packet, int pktType) {
        ...
        packet.position(packet.position() + (16 - addrLen) + 64 // skip server
                                                                // host name (64
                                                                // chars)
        ...
        while ((packet.position() < packet.limit()) && notFinishedOptions) {
            byte optionTypeByte = (byte) (packet.get() & 0xFF);
            short optionType;
            if (optionTypeByte < 0) {
                optionType = (short) (optionTypeByte & 0x7F + (1 << 7));
            } else {
                optionType = optionTypeByte;
            }
            if (optionTypeByte == (byte) 0xFF) {
                notFinishedOptions = false;
            } else {
                byte optionLen = packet.get();
                int expectedLen = 0;
                LOG.debug("DhcpPacket: decodeFullPacket : OptionType = "
                        + optionType);
                switch (optionType) {
                    ...
                    case DHCP_MUD_URL :
                        expectedLen = optionLen;
                        mudUrl = readAsciiString(packet, optionLen);
                        LOG.debug("MUD URL = " + mudUrl);
                        break;
                    default :
        ...
        switch (dhcpType) {
            ...
            case DHCP_MESSAGE_TYPE_DISCOVER :
                newPacket = new DhcpDiscoverPacket(transactionId, clientMac,
                        broadcast);
                break;
            case DHCP_MESSAGE_TYPE_OFFER :
                newPacket = new DhcpOfferPacket(transactionId, broadcast, ipSrc,
                        yourIp, clientMac);
                break;
            case DHCP_MESSAGE_TYPE_REQUEST :
                newPacket = new DhcpRequestPacket(transactionId, clientIp,
                        clientMac, broadcast);
                ((DhcpRequestPacket) newPacket).setMudUrl(mudUrl);
                break;
            case DHCP_MESSAGE_TYPE_DECLINE :
                newPacket = new DhcpDeclinePacket(transactionId, clientIp,
                        yourIp, nextIp, relayIp, clientMac);
                break;
            case DHCP_MESSAGE_TYPE_ACK :
                newPacket = new DhcpAckPacket(transactionId, broadcast, ipSrc,
                        yourIp, clientMac);
                break;
            case DHCP_MESSAGE_TYPE_NAK :
                newPacket = new DhcpNakPacket(transactionId, clientIp, yourIp,
                        nextIp, relayIp, clientMac);
                break;
            case DHCP_MESSAGE_TYPE_INFORM :
                newPacket = new DhcpInformPacket(transactionId, clientIp,
                        yourIp, nextIp, relayIp, clientMac);
                break;
            default :
    ...
    public static ByteBuffer buildDiscoverPacket(int encap, int transactionId,
        ...
        DhcpPacket pkt = new DhcpDiscoverPacket(transactionId, clientMac,
                broadcast);
    ...
    public static ByteBuffer buildOfferPacket(int encap, int transactionId,
            boolean broadcast, InetAddress serverIpAddr,
            InetAddress clientIpAddr, byte[] mac, Integer timeout,
            InetAddress netMask, InetAddress bcAddr, InetAddress gateway,
            List<InetAddress> dnsServers, InetAddress dhcpServerIdentifier,
            String domainName) {
        DhcpPacket pkt = new DhcpOfferPacket(transactionId, broadcast,
                serverIpAddr, clientIpAddr, mac);
    ...
    public static ByteBuffer buildAckPacket(int encap, int transactionId,
            ...
            List<InetAddress> dnsServers, InetAddress dhcpServerIdentifier,
            String domainName) {
        DhcpPacket pkt = new DhcpAckPacket(transactionId, broadcast,
                serverIpAddr, clientIpAddr, mac);
        pkt.mGateway = gateway;
        pkt.mDnsServers = dnsServers;
        pkt.mLeaseTime = timeout;
     ...
     */
    public static ByteBuffer buildRequestPacket(int encap, int transactionId,
            InetAddress clientIp, boolean broadcast, byte[] clientMac,
            InetAddress requestedIpAddress, InetAddress serverIdentifier,
            byte[] requestedParams, String hostName) {
